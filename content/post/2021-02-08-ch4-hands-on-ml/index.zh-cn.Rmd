...
所以岭回归的正则化项就是$\alpha 1/2(||w||_2)^2$,w是$\theta_1$到$\theta_n$的参数向量(特征权重)
...
上面那个损失函数在$\theta_i=0$的地方是不可微分的，但是可以通过将梯度向量替换成次梯度向量(subgradient vector)来解决这个问题：
$$
