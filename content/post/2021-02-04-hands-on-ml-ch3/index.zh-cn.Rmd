---
title: 【Ch3】Hands on Machine Learning
author: wutao
date: '2021-02-04'
slug: hands_on_ml_ch3
categories:
  - reading notes
tags:
  - notes
...
通过 Scikit-Learn下载的数据是字典的结构，包含key和value，比如`DESCR`key表示数据集的描述，`data` key表示数据集，`target` key表示数据集的标签
...
上面的例子说明：仅仅使用准确度来衡量模型是不太好的，特别是对于有偏向性的数据(skewed datasets)
...
这个混淆矩阵可以使用下图来表示： ![](https://picgo-wutao.oss-cn-shanghai.aliyuncs.com/img/%E6%BC%94%E7%A4%BA%E6%96%87%E7%A8%BF1_01.png)
...
-   召回率(recall)(或者叫灵敏度sensitivity; 真阳性率FPR)表示 在真实的positive里面预测是positive的比例：
...
-   一对多策略(one-versus-the-rest (OvR)): 比如要将手写图片分为0-9一共10个类别，那么我们就可以训练10个分类器，每个分类器处理的是一个二分类问题(属于这一类还是不属于这一类)，都可以得到一个score，对于每个图片就选择10个分类器中score最高的分类器所对应的类作为该图片的预测类

-   一对一策略(one-versus-one (OvO)):对所有的类两两组合训练二分类的分类器，如果有N类，那么就需要训练N\*(N-1)/2个分类器，对于一个图片就需要运行所有的分类器(10类别是45个)，在这些结果中预测次数最多的类就是该图片的预测类，这个方法的好处是在训练时只需要对一部分训练数据进行训练(只涉及要识别的类的数据，比如0-1分类器只需要对所有的0/1图片进行训练)
